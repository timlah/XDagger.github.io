...
loader.load(
  // resource URL
  '/XDagger.github.io/assets/objects/gpu.obj',
...
function init(object) {
  ...
  root.camera.position.set(-200, -100, 400);
  root.controls.target.set(250, 75, 0);
  ...
  var animationObject = new Animation({
    path: createPointsFrom(object, 1, 8),
    scale: 0.5,
    particles: 20000,
    color: {
      h: function(i, count) {
        return 0.35 + (0.4 * utils.zeroOneZero(i / count));
      },
      s: function() {
  ...
  animationObject.animate(200.0, {ease: Power0.easeIn, repeat:-1});
